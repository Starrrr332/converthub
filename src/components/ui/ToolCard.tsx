  ...
  return (
    <Link to={to} className={`card-interactive group block h-full ${compact ? 'p-5' : ''}`}>
      <div className="flex items-start justify-between gap-3">
        <div
          className={`inline-flex p-3 rounded-xl ${color} group-hover:scale-105 transition-transform duration-300`}
        >
      ...
      <h3 className={`font-semibold text-slate-900 mt-4 ${compact ? 'text-base' : 'text-lg'}`}>
        {title}
      </h3>
