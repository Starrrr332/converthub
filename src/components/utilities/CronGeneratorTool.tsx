    ...
    <div>
      ...
      <div className="grid grid-cols-5 gap-2 mb-4">
        {[
          ...
          { id: 'dow', label: 'Día semana', val: dow, set: setDow, placeholder: '*' },
        ].map((f) => (
          <div key={f.id}>
            ...
            <input
              ...
              className="w-full p-2 border-2 border-gray-200 rounded-lg text-sm text-center"
            />
          </div>
        ))}
      </div>
        ...
        <div className="mt-4 p-4 bg-gray-50 rounded-lg">
          ...
          <p className="text-sm text-gray-500 text-center mt-2">
            {minute === '*' && hour === '*' && dom === '*' && month === '*' && dow === '*'
              ? 'Cada minuto'
              : dom !== '*'
                ? `Se ejecuta el día ${dom} a las ${hour}:${minute}`
                : hour !== '*'
                  ? `Se ejecuta a las ${hour}:${minute}`
