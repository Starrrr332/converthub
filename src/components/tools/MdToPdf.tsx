...
function renderMarkdown(md: string): string {
  let html = md
    .replace(
      /```([\s\S]*?)```/g,
      '<pre class="bg-slate-100 rounded-lg p-4 font-mono text-sm overflow-x-auto my-4"><code>$1</code></pre>',
    )
    .replace(
      /`([^`]+)`/g,
    ...
    .replace(
      /^## (.+)$/gm,
      '<h2 class="text-2xl font-bold mt-8 mb-4 text-slate-900 border-b border-slate-200 pb-2">$1</h2>',
    )
    ...
    .replace(
      /^> (.+)$/gm,
      '<blockquote class="border-l-4 border-indigo-300 pl-4 italic text-slate-600 my-2">$1</blockquote>',
    )
