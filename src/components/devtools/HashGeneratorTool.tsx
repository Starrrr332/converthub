...
export function HashGeneratorTool() {
  ...
  function simpleMD5(string: string): string {
    ...
    tail[14] = n * 8;
    md5cycle(state, tail);
    return state.map(b => (b & 0xFFFFFFFF).toString(16).padStart(8, '0')).join('');
  }
    ...
    <div className="space-y-4">
      <div>
        ...
        <textarea
          ...
          className="input-field h-32 font-monic text-sm"
        />
      </div>

      <Button onClick={generateHashes} disabled={!input} className="w-full">
      ...
      {Object.keys(results).length > 0 && (
        <div className="space-y-2">
          {Object.entries(results).map(([algo, hash]) => (
            <div key={algo} className="flex items-center gap-2 p-3 bg-gray-50 rounded-lg">
              <span className="text-xs font-bold text-gray-500 w-16">{algo}</span>
              <code className="flex-1 text-xs font-mono text-gray-700 break-all">{hash}</code>
              <button
