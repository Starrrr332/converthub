...
export async function splitPdf(options: PdfSplitOptions): Promise<PdfConversionResult[]> {
  ...
  if (options.splitAll) {
    ...
    for (const range of options.pageRanges) {
      ...
      results.push({ blob, url, filename: `pages-${range.start}-${range.end}.pdf`, size: blob.size });
    }
  }
...
export async function watermarkPdf(options: PdfWatermarkOptions): Promise<PdfConversionResult> {
  ...
  for (const page of pages) {
    ...
    if (position === 'top-left') { x = 100; y = height - 100; }
    if (position === 'top-right') { x = width - 100; y = height - 100; }
    if (position === 'bottom-left') { x = 100; y = 100; }
    if (position === 'bottom-right') { x = width - 100; y = 100; }
