...
const categories: Record<Category, { label: string; units: UnitDef[] }> = {
  ...
  volume: {
    ...
    units: [
      ...
      { id: 'floz', label: 'Onzas líquidas', toBase: v => v * 0.0295735, fromBase: v => v / 0.0295735 },
  ...
  speed: {
    ...
    units: [
      ...
      { id: 'c', label: 'Velocidad luz', toBase: v => v * 299792458, fromBase: v => v / 299792458 },
    ]
  },
  area: {
    label: 'Área',
              ...
              {cat.units.slice(0, 6).map(u => {
                ...
                return (
                  <div key={u.id} className="flex justify-between p-1">
                    ...
                    <span className="font-medium">{u.fromBase(baseValue).toLocaleString(undefined, { maximumSignificantDigits: 6 })}</span>
                  </div>
                );
              })}
            </div>
