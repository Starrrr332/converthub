...
vi.mock('../../src/utils/fileHelpers', () => ({
  ...
  createCanvasFromImage: vi.fn().mockImplementation(() => ({
    width: 800,
    height: 600,
...
describe('convertImage', () => {
  ...
  it('applies quality setting', async () => {
    ...
    expect(canvasToBlob).toHaveBeenCalledWith(expect.anything(), 'image/jpeg', 50);
  });
  ...
  it('resizes without aspect ratio', async () => {
    const file = createMockFile('test.png', 'image/png');
    const result = await convertImage(file, {
      format: 'image/png',
      width: 500,
  ...
  it('applies both flip directions', async () => {
    const file = createMockFile('test.png', 'image/png');
    await convertImage(file, {
      format: 'image/png',
      flip: { horizontal: true, vertical: true },
    });
    expect(mockContext.translate).toHaveBeenCalledWith(800, 600);
    expect(mockContext.scale).toHaveBeenCalledWith(-1, -1);
  });
});
...
describe('convertBatch', () => {
  ...
  it('calls onProgress callback', async () => {
    ...
    const files = [createMockFile('a.png', 'image/png'), createMockFile('b.png', 'image/png')];
